...
- **`GeneratedAsset`** 表: 系统的金矿。记录每一次历尽艰辛求出的 AI 渲染成果。
  - 主要字段：`id`, `userId` (关联), `originalImage` (原图 blob url), `resultImage` (正面手办主视觉), `backImage` (背面视图), `sideImage` (侧面视图), `showcaseImage` (效果展示图, 可空), `prompt` (生图提示词), `baseModelVariantId` (当时套用在哪个白模地基上的校验码)。
  - **重要**: 其带有 `createdAt` 用来做成本对账。`showcaseImage` 为可空字段，效果展示图生成失败时为 `null`。
